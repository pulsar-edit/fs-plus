...
let BINARY_EXTENSIONS, COMPRESSED_EXTENSIONS, IMAGE_EXTENSIONS, MARKDOWN_EXTENSIONS, checkIfElectron2OrLower, isElectron2OrLower, isMoveTargetValid, isMoveTargetValidSync, isPathValid, lstatSyncNoException, statSyncNoException, fsPlus, __slice = [].slice;
...
fsPlus = {
  ...
  */
  tildify: function(pathToTildify) {
    ...
    let normalized = fsPlus.normalize(pathToTildify);
    let homeDir = fsPlus.getHomeDirectory();
    if (homeDir == null) {
      return pathToTildify;
    }
  ...
  */
  isDirectorySync: function(directoryPath) {
    ...
    let stat = statSyncNoException(directoryPath);
    if (stat) {
  ...
  */
  isFileSync: function(filePath) {
    ...
    let stat = statSyncNoException(filePath);
    if (stat) {
      return stat.isFile();
    } else {
      return false;
  ...
  */
  isSymbolicLinkSync: function(symlinkPath) {
    ...
    let stat = lstatSyncNoException(symlinkPath);
    if (stat) {
  ...
  */
  isSymbolicLink: function(symlinkPath, callback) {
    if (isPathValid(symlinkPath)) {
      fs.lstat(symlinkPath, function(error, stat) {
  ...
  */
  isExecutableSync: function(pathToCheck) {
    ...
    let stat = statSyncNoException(pathToCheck);
    if (stat) {
